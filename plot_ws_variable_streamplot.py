...
# 创建网格点
y, x = np.mgrid[0:wind_data[(0, 0)]['speed'].shape[0], 0:wind_data[(0, 0)]['speed'].shape[1]]
...
for i in range(num_levels):
    for j in range(num_times):
        ...
        # 添加流场（只在非掩码位置）
        # 对数据进行降采样以提高可视化效果
        sample_step = 5  # 可以调整这个值来改变流线密度
        ax.streamplot(x[::sample_step, ::sample_step], 
                     y[::sample_step, ::sample_step],
                     u[::sample_step, ::sample_step],
                     v[::sample_step, ::sample_step],
                     color='black',
                     density=1.5,
                     linewidth=0.5,
                     arrowsize=0.5)
...
# 保存图片
plt.savefig('picture/wind_speed_streamplot.png', dpi=600, bbox_inches='tight')
plt.close()

# 关闭数据文件
data.close()

print("成功保存所有高度和时间步长的流场图片到picture/wind_speed_streamplot.png") 